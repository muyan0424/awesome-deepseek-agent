...
Qoder 是一款 Agentic Coding 产品，支持 IDE、CLI 和 JetBrains Plugin三种形态。Qoder 内置支持 DeepSeek 模型，也支持通过 API 密钥接入 DeepSeek（自定义模型）。
...
DeepSeek V4 系列模型支持最高 **100 万 token 上下文**与 **max 级别思考强度**。选定模型后，可在模型选择器中为其设置上下文窗口与思考强度，建议选择 1M 上下文并将思考强度设为 max，以获得最佳编程体验。
